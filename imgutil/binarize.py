...
def main():
    if len(sys.argv) != 2:
        print("Usage: python -m imgutil.binarize <input.png>")
        sys.exit(1)
    ...
    # Convert to grayscale and then apply a binary threshold
    ...
    # Save the binary image. Use cv2.imencode/ndarray.tofile so
    # that writing also works with non-ASCII paths.
    success, buffer = cv2.imencode(input_path.suffix, binary)
    if not success:
